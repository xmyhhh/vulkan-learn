...
void TextureApplication::drawFrame()
{
...
void TextureApplication::mainLoop()
{
...
void TextureApplication::createCommandBuffer()
{
...
void TextureApplication::createSyncObjects()
{
...
void TextureApplication::createDescriptorSets()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
...

		{
			...
			descriptorWrites[0].dstArrayElement = 0;//表示写入Array中的第几个对象
...
void TextureApplication::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
void TextureApplication::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
